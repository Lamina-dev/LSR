...
`let`, `var`, `func`, `return`, `in`, `not`, `loop`, `while`, `break`, `continue`, `if`, `else`, `match`, `sym`, `import`, `use`, `as`
...
本规范移除 `:` 类型标注，变量声明统一使用 `let name = expr`。
...
本节用于明确 `vector`、`matrix`、`table`、`set` 在赋值与函数传参时的行为。

#### 4.6.1 总规则

- 复合类型在实现层采用**共享引用**保存对象，以避免不必要拷贝
- 语言可观察语义采用**写时复制（COW）**保证可预测性
- 只读操作（读取、遍历、计算）不触发复制
...
为保障类型稳定性与 SIMD/BLAS 友好性，`vector` 与 `matrix` 采用强齐次约束。
...
- 量纲签名在**编译期类型检查阶段**表示为 SI 七维整数向量：`[M, L, T, I, Θ, N, J]`
- 无量纲纯数字对应零向量
- 乘除运算对应向量加减
...
`Expr` 表示延迟求值 AST。数值运算中混入 `Expr` 将整体提升为 `Expr`。
...
`import`：加载模块并以模块对象挂载到当前作用域。  
`use`：从模块导入指定符号到当前作用域（冲突时禁止覆盖）。
