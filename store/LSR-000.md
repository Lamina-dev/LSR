...
- `num` 目前为概念类型，但其下`int`, `real`, `frac`均为实际类型
- Lamina没有浮点数，浮点数只存在于FFI操作中，解释器使用分数模拟有理数运算，无精度损失


### 4.5 可空类型
写法：`T?`，表示可能为null, 对于上文定义的值类型它是无效多余的，编译器也不会让T为值类型，用于判断对象的引用是否为空引用
必须处理空情况才能正常使用
...
- 单位名在当前命名空间内唯一
- 单位定义要求无环（如 `unit a = 2<b>` 且 `unit b = 3<a>` 形成环）
