...
本规范定义 Lamina 标准库（`std`）的模块结构、导入与调用规则、数学核心 API 和错误模型。
...
- 提供统一、稳定、可组合的标准 API
- 与 LSR-000 的静态强类型与单位系统一致
- 优先覆盖数学系统核心能力（数值、线代、统计、随机、单位）
- 保证同名函数跨平台语义一致
...
说明：`strip` 仅是函数式入口，不引入新语义，语义以 LSR-008 为准。
