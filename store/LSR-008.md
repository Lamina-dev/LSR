...
本规范定义带量纲数值到无量纲纯数值的标准路径，覆盖 `as num` 与 `as scalar` 两种语义。
...
- 例如：`10<km> as num` 的结果是 `10000`

#### 3.2 目标单位下剥离

- 若用户希望在某单位刻度下得到纯数值，必须先显式转换单位
- 例如：`(10<km> as <km>) as num` 结果为 `10`

#### 3.3 `as scalar` 语义

- `expr as scalar` 保留当前显示刻度，仅移除单位标签
- 例如：`10<km> as scalar` 结果为 `10`
...
说明：外部函数若明确要求“保留当前单位刻度的数值”，可使用 `as scalar`。

### 6. 安全约束与错误模型

- `UnitStripTypeMismatch`: 作用于非数值类型
- `UnitStripOverflow`: 剥离过程发生表示范围溢出（实现相关）
- `UnitStripInvalid`: 单位转换阶段失败（例如无效目标单位）
